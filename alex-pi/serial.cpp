...
void startSerial(const char *portName, int baudRate, int byteSize, char parity, int stopBits, int maxAttempts) {
    ...
    if (_fd < 0) perror("GIVING UP. Unable to open serial port.");
    else {
        fcntl(_fd, F_SETFL, 0);
        ...
        if (stopBits == 2) _serOptions.c_cflag |= CSTOPB;
        else _serOptions.c_cflag &= ~CSTOPB;
...
int serialRead(char *buffer) {
    ssize_t n = 0;
    if (_fd >= 0) n = read(_fd, buffer, MAX_BUFFER_LEN);
    return n;
}

void serialWrite(char *buffer, int len) {
    ssize_t n = -1;
    if (_fd >= 0) n = write(_fd, (void *) buffer, len);
}
