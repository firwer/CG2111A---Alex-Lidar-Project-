...
/**
 * Data size is 4 + 128 + 4 + 1 = 137 bytes.
 * We pad to 140 bytes as this is the nearest divisible by 4 we have. So we add 3 bytes.
 */
...
static TResult assemble(char *outputBuffer, const char *inputBuffer, int len) {
    ...
    if (leftoverFlag) {
        ...
        for (i = 0; i < copyCount; i++) outputBuffer[counter++] = leftoverBuffer[i];
    }
